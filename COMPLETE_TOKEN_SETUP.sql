...
-- Run this in Supabase SQL Editor to add all missing token components
...
-- Step 2: Add missing columns to live_competitions table
...
-- Step 3: Create token_transactions table
...
CREATE INDEX IF NOT EXISTS idx_token_transactions_competition ON token_transactions(competition_id);
CREATE INDEX IF NOT EXISTS idx_users_last_token_claim ON users(last_token_claim);

-- Step 5: Enable Row Level Security on token_transactions
ALTER TABLE token_transactions ENABLE ROW LEVEL SECURITY;

-- Step 6: Create policies for token_transactions
...
-- Step 7: Create claim hourly tokens function (100,000 tokens per hour)
...
BEGIN
    ...
    hours_since_last_claim := EXTRACT(EPOCH FROM (NOW() - user_record.last_token_claim)) / 3600.0;
    ...
    -- 100,000 tokens per hour, cap at 24 hours worth (2.4M max)
...
-- Step 8: Create game entry fee function (20,000 tokens)
...
-- Step 9: Create winnings distribution function
...
-- Step 10: Create get user token info function
...
BEGIN
    ...
    hours_until_next_claim := 1 - (EXTRACT(EPOCH FROM (NOW() - user_record.last_token_claim)) / 3600.0);
...
-- Step 11: Give existing users starting tokens (ONLY ONCE - add has_received_bonus flag)
ALTER TABLE users ADD COLUMN IF NOT EXISTS has_received_starting_bonus BOOLEAN DEFAULT false;

-- Give starting bonus only to users who haven't received it yet
UPDATE users 
SET chump_tokens = 500000, has_received_starting_bonus = true 
WHERE (chump_tokens IS NULL OR chump_tokens < 500000) AND has_received_starting_bonus = false;

-- Set new user default with bonus flag
ALTER TABLE users ALTER COLUMN has_received_starting_bonus SET DEFAULT true;

-- SUCCESS MESSAGE
SELECT 'Token system setup complete! Users get 500K starting tokens and 100K per hour.' as status; 