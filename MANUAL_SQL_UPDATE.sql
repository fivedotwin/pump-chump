-- MANUAL SQL UPDATE FOR SUPABASE DASHBOARD
-- Copy and paste this into https://supabase.com/dashboard -> SQL Editor

-- Step 1: Update hourly bonus function to give 100,000 tokens per hour
...
-- Step 2: Update game entry fee function to charge 20,000 tokens
...
-- Step 3: Update default entry cost for new competitions
ALTER TABLE live_competitions ALTER COLUMN entry_cost SET DEFAULT 20000;

-- Step 4: Update existing competitions entry cost
UPDATE live_competitions SET entry_cost = 20000 WHERE entry_cost = 200;

-- Step 5: Give all existing users a starting bonus (500,000 tokens minimum)
UPDATE users SET chump_tokens = GREATEST(chump_tokens, 500000) WHERE chump_tokens < 500000;

-- Step 6: Update new user default starting tokens
ALTER TABLE users ALTER COLUMN chump_tokens SET DEFAULT 500000; 